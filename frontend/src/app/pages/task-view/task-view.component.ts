...
export class TaskViewComponent implements OnInit {
  ...
  onClickUpdateList(list: List) {
    let data = prompt("Update List Title:", list.title);
    if (data !== null) {
      if (data.length >= 3) {
        this.webService.updateList(list._id, data).subscribe(
          (updatedList: List) => {
            const index = this.lists.findIndex(l => l._id === list._id);
            if (index !== -1) {
              this.lists[index].title = data!;
            }
          },
          (error) => {
            console.error(error);
  ...
  onClickUpdateTask(task: Task) {
    let data = prompt("Update Task Title:", task.title);
    if (data !== null) {
      if (data.length >= 3) {
        this.webService.updateTaskTitle(this.listId, task._id, data).subscribe(
          (updatedTask: Task) => {
